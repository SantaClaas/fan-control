 ...
 * @param {number} final
 */
function cubicBezier2Point(t, initial, p1, p2, final) {
  return (
    (1 - t) ** 3 * initial +
    3 * (1 - t) ** 2 * t * p1 +
    3 * (1 - t) * t ** 2 * p2 +
...
/**
 * Based on https://blog.webdevsimplified.com/2022-03/debounce-vs-throttle/
 * @param {Function} callback
 * @param {number} delayMilliseconds
 * @returns {(...args: any[]) => void}
 */
...
function FanControl() {
  ...
  /** @type {import("solid-js").Signal<Animation | undefined>} */
  ...
  createEffect(() => {
    ...
    // When the animation speed is a minute, the playback rate is the RPM
    animation.updatePlaybackRate(rpm());
  });

  /** @type {SVGSVGElement | undefined} */
  let fan;

  // Animations need to start after the component is mounted and rendered to the DOM
  // Starting the animation in the ref callback doesn't work as the element is created but not yet rendered
  onMount(() => {
    if (!fan) return;
      ...
      <main class="grid h-[100dvh] grid-rows-[auto_1fr_auto_auto] gap-4 bg-cyan-950 p-4 text-slate-50">
        ...
        <section class="overflow-hidden">
          ...
          <svg
            ...
            width="24"
            fill="currentColor"
            class="h-full w-full rounded-full fill-cyan-300/30"
            ref={fan}
          >
